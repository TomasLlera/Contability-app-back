...
router.post('/', requireAdmin, audit('rubro'), asyncHandler(async (req, res) => {
...
router.put('/:id', requireAdmin, audit('rubro'), asyncHandler(async (req, res) => {
  await db.updateRubro(req.params.id, req.body.nombre, req.body.icon);
  res.json({ ok: true });
}));

router.delete('/:id', requireAdmin, audit('rubro'), asyncHandler(async (req, res) => {
  await db.deleteRubro(req.params.id);
  res.json({ ok: true });
}));

router.delete('/:id/movimientos', requireAdmin, audit('movimientos_bulk_rubro'), asyncHandler(async (req, res) => {
...
router.put('/:id/import-config', requireAdmin, audit('rubro_import_config'), asyncHandler(async (req, res) => {
